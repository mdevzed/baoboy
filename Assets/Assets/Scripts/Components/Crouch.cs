...
public class Crouch : MonoBehaviour
{
    ...
    public float minCrouchYHeadPosition = 0.5f; // New: Minimum head Y position
    ...
    void LateUpdate()
    {
        if (Input.GetKey(key))
        {
            ...
            if (headToLower)
            {
                ...
                float targetY = Mathf.Clamp(headToLower.localPosition.y - 0.01f, minCrouchYHeadPosition, defaultHeadYLocalPosition.Value);
                headToLower.localPosition = new Vector3(headToLower.localPosition.x, targetY, headToLower.localPosition.z);
            }
            ...
            if (colliderToLower)
            {
                ...
                if(defaultHeadYLocalPosition.HasValue)
                {
                    loweringAmount = defaultHeadYLocalPosition.Value - headToLower.localPosition.y;
                }
                else
                {
